...
class checkpoint():
    def __init__(self, args):
        ...
        if not args.create_results:
            ...
            _make_dir(args.test_dir + '/' + args.arch)
            _make_dir(args.test_dir + '/' + args.arch + '/' + args.testset)
            _make_dir(args.test_dir + '/' + args.arch + '/' + args.testset + '/' + 'X' + str(args.scale[0]))
            open_type = 'a' if os.path.exists(self.dir + '/psnr_log.txt') else 'w'
            self.psnr_log_file = open(self.args.test_dir + '/' + self.args.arch + '/' + self.args.testset + '/' + 'X' + str(self.args.scale[0]) + '/psnr.txt', open_type)

    def write_psnr(self, log, refresh=False):
        self.psnr_log_file.write(log + '\n')
        if refresh:
            self.psnr_log_file.close()
            self.psnr_log_file = open(self.args.test_dir + '/' + self.args.arch + '/' + self.args.testset + '/' + 'X' + str(self.args.scale[0]) + '/psnr.txt', 'a')
    ...
    def create_results(self, filename, save_list, scale):
        filename = '{}/{}/{}/X{}/{}_x{}_'.format(self.dir, self.args.arch, self.args.testset, scale, filename, scale)
        postfix = ('SR', 'LR', 'HR')
        for v, p in zip(save_list, postfix):
            normalized = v[0].data.mul(255 / self.args.rgb_range)
            ndarr = normalized.byte().permute(1, 2, 0).cpu().numpy()
